...
The database design supports administrative functions:

1. **User Management**
   - Admin users can view, edit, and manage user accounts
   - Role-based access control is determined by the `role` field in the `users` table

2. **Sales Analytics**
   - The system can generate reports by querying `orders` and `order_items` tables
   - Data can be aggregated to show:
     - Top-selling games
     - Sales trends over time
     - Revenue by game category

3. **Inventory Control**
   - Low stock alerts can be generated by monitoring the `inventory` table
   - Purchase recommendations can be made based on sales velocity